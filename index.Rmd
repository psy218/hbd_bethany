...
# Methods 
## Participants  
  Participants were recruited through a slack channel and e-mails; see Figure \@ref(fig:recruitment) for an example recruitment material. The final sample of XX Bethany Affiliates was mostly good people; see [Appendix](#appendix) for the full list of participants.   
...
We first present the descriptive statistics of the birthday messages. The number of words in the birthday messages ranged from `r wc_stats$min` to `r wc_stats$max` words with an average of `r round(wc_stats$mean)` ($SD$ = `r wc_stats$sd`) words. A message with the highest number of words was from `r stringr::str_to_title(most_words$sender)` and was `r stringr::str_to_title(most_words$word_count)`-words long. See Figure \@ref(fig:word-count-by-sender) for the word counts for each Bethany affiliate.    
